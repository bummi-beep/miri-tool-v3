...
// if hold is true then leave ISP asserted on exit.
{
  ...
  //  gpio_pad_select_gpio(12);
    gpio_set_direction(11, GPIO_MODE_INPUT_OUTPUT);
    ...
    // 1. 통신 에코 설정 (NXP 칩 부트로더 기본 사양)
    cfg->echo = 1;

    // 2. 전용 제어 핀을 출력 모드로 설정
    gpio_set_direction(cfg->ispPin, GPIO_MODE_OUTPUT);
    gpio_set_direction(cfg->resetPin, GPIO_MODE_OUTPUT);

    // ==========================================================
    // 3. 정석적인 ISP 진입 시퀀스 시작
    // ==========================================================

    // Step A: ISP 핀을 LOW로 설정하여 부트로더 진입 조건을 미리 만듦
    gpio_set_level(cfg->ispPin, 1);

    // Step B: 타겟 MCU를 리셋 상태(LOW)로 빠뜨림
    gpio_set_level(cfg->resetPin, 1);

    // 핀 상태가 전기적으로 완전히 안정화될 때까지 대기 (예: 50ms)
    vTaskDelay(50 / portTICK_PERIOD_MS);

    // Step C: 리셋 해제 (HIGH) -> 이 순간 타겟 MCU가 부팅을 시작함!
    // 타겟 MCU는 내부적으로 "어? 내 ISP 핀이 LOW네? 부트로더로 켜야지!" 라고 판단함.
    gpio_set_level(cfg->resetPin, 1);

    // 칩의 내부 부트로더가 완전히 메모리에 로드되고 통신 준비가 될 때까지 넉넉히 대기
    ...
    // hold가 0(거짓)이면 부팅이 성공적으로 끝났으므로 ISP 핀을 다시 HIGH로 원복함.
    if (!hold) {
        gpio_set_level(cfg->ispPin, 1);
    }
...
//   <-- OK
{
    ...
    while (retry--) {
        ...
        ret = LPCISP_SERIAL_ReadBytes(fd, (unsigned char *) buffer,ARRAY_SIZE(buffer), 1000);
...
// erase a range of sectors.  return 0 on success, -1 on failure
{
    ...
    if ((startSector >= 0) && (endSector >= startSector)) {
        if (PrepareSectorsForWrite(fd, cfg, startSector, endSector, bank, partInfo)) {
            if (partInfo->numBanks > 1) {
...
{
    unsigned int
            bytesToWrite = 0; // 초기화 추가
    int
            fail;
    int
            curSector;
    unsigned char
            *buffer    = NULL; // use a secondary buffer so we can fill the tail of short writes with 0xff
    unsigned int total = length;
    int write_extra_block = 1; // 마지막 블록 이후 추가 블록 쓰기 플래그

    fail = 0;
    ReportString(REPORT_INFO, "writing... 0x%08x flashblocksize %d\n", addr, partInfo->flashBlockSize);

    buffer = (unsigned char *) heap_caps_malloc(partInfo->flashBlockSize,MALLOC_CAP_SPIRAM);
    if (buffer) {
        while (!fail && (length || write_extra_block)) {
            if (length > 0) {
                // 일반 데이터 블록 쓰기
            ...
            } else if (write_extra_block) {
                // 추가 블록 쓰기 (0xff로 채움)
                bytesToWrite = partInfo->flashBlockSize; // 전체 블록 크기로 설정
                memset(buffer, 0xff, partInfo->flashBlockSize);
                write_extra_block = 0; // 추가 블록은 한 번만 쓰기
                ReportString(REPORT_DEBUG_PROCESS, "writing extra 0xFF block at 0x%08x\n", addr);
            }
                ...
                if (curSector >= 0) {
                    if (PrepareSectorsForWrite(fd, cfg, curSector, curSector, 0, partInfo)) // @@@ only support bank 0 for now
                    {
                        ...
                        if (LPCISP_CopyRAMtoFlash(fd, cfg, partInfo->flashBlockRAMBase, addr, partInfo->flashBlockSize)) {
                            ...
                            addr += partInfo->flashBlockSize;
                            ReportString(REPORT_INFO, "\b\b\b\b\b\b\b\b%08x\n", addr);

                            /* erase 구간은 0~50% (uart_isp_erase_all),
                             * write 구간은 50~100% 를 사용한다. */
                            if (total > 0) {
                                unsigned int written = total - length;
                                int percent = 50 + (int)((written * 50U) / total);
                            ...
                            ReportString(REPORT_ERROR, "failed to write %d bytes from 0x%08x to 0x%08x", bytesToWrite, partInfo->flashBlockRAMBase, addr);
                            fail = 1;
                        }
                    }
                    else {
                        ReportString(REPORT_ERROR, "failed to prepare to write to sector %d", curSector);
                        fail = 1;
                    }
                }
                else {
                    ReportString(REPORT_ERROR, "failed to find a sector which contains address 0x%08x", addr);
                    fail = 1;
                }
            }
            else {
                ReportString(REPORT_ERROR, "failed to copy %d bytes to RAM", bytesToWrite);
                fail = 1;
            }
        }
